...
df = pd.DataFrame({"extension": ds-b0, "force": fs/603, "rate": morse_ks})
...
```{python}
df = df.melt(id_vars=["force"], value_vars=["rate", "hyd_rate"])
df['variable'] = df['variable'].map({'rate': 'homolysis', 'hyd_rate': 'hydrolysis'})
```

```{python}
p = (
    ggplot(df, aes(x="force", y="value", color="variable"))
    + geom_line(size=1.3)
