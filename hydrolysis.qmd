...
path = f"{run}/2_pull/distances.dat"
distances = read_distances_dat(Path(path))
...
for bondkey, plumed_id in bond_to_plumed_id.items():
  bond_distances[bondkey] = []
for time in distances.keys():
  for bondkey, plumed_id in bond_to_plumed_id.items():
    bond_distances[bondkey].append(distances[time][plumed_id])
```
...
df['r0'] = r0
df['key'] = str(key)
df.to_csv("./results/example-bond-lengths.csv")
