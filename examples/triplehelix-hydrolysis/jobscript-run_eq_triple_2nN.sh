#!/bin/env bash
#SBATCH --job-name=run_eq_triple_2nN
#SBATCH --output=run_eq_triple_2nN.slurm.out
#SBATCH --error=run_eq_triple_2nN.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_eq_triple_2nN.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_eq_triple_2nN.sh
  exit 2
fi