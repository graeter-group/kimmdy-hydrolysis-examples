#!/bin/env bash
#SBATCH --job-name=run_eq_triple_0nN
#SBATCH --output=run_eq_triple_0nN.slurm.out
#SBATCH --error=run_eq_triple_0nN.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_eq_triple_0nN.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_eq_triple_0nN.sh
  exit 2
fi