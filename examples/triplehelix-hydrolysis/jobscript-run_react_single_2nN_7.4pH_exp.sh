#!/bin/env bash
#SBATCH --job-name=run_react_single_2nN_7.4pH_exp
#SBATCH --output=run_react_single_2nN_7.4pH_exp.slurm.out
#SBATCH --error=run_react_single_2nN_7.4pH_exp.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_just_reactions_single_2nN_7.4pH_exp.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_react_single_2nN_7.4pH_exp.sh
  exit 2
fi