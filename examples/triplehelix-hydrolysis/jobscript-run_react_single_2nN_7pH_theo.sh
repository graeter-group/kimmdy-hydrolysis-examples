#!/bin/env bash
#SBATCH --job-name=run_react_single_2nN_7pH_theo
#SBATCH --output=run_react_single_2nN_7pH_theo.slurm.out
#SBATCH --error=run_react_single_2nN_7pH_theo.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_just_reactions_single_2nN_7pH_theo.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_react_single_2nN_7pH_theo.sh
  exit 2
fi