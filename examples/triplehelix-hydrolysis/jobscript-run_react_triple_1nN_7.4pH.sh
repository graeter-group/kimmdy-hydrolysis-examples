#!/bin/env bash
#SBATCH --job-name=run_react_triple_1nN_7.4pH
#SBATCH --output=run_react_triple_1nN_7.4pH.slurm.out
#SBATCH --error=run_react_triple_1nN_7.4pH.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_just_reactions_triple_1nN_7.4pH.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_react_triple_1nN_7.4pH.sh
  exit 2
fi