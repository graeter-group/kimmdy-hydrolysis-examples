#!/bin/env bash
#SBATCH --job-name=run_eq_collagen_0.2nN
#SBATCH --output=run_eq_collagen_0.2nN.slurm.out
#SBATCH --error=run_eq_collagen_0.2nN.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_eq_collagen_0.2nN.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_eq_collagen_0.2nN.sh
  exit 2
fi