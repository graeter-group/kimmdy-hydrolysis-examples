#!/bin/env bash
#SBATCH --job-name=run_eq_collagen_1nN
#SBATCH --output=run_eq_collagen_1nN.slurm.out
#SBATCH --error=run_eq_collagen_1nN.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_eq_collagen_1nN.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_eq_collagen_1nN.sh
  exit 2
fi