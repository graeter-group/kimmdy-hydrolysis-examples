#!/bin/env bash
#SBATCH --job-name=run_eq_collagen_0.2nN_shear
#SBATCH --output=run_eq_collagen_0.2nN_shear.slurm.out
#SBATCH --error=run_eq_collagen_0.2nN_shear.slurm.err
...
timeout ${CYCLE_buffered}h kimmdy -i kimmdy_eq_collagen_0.2nN_shear.yml --restart
...
if [ $HOURS -lt $CYCLE ]; then
  ...
  sbatch ./jobscript-run_eq_collagen_0.2nN_shear.sh
  exit 2
fi